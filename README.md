...
It returns the latest posts as JSON and rewrites legacy links to `infinite-curios.pages.dev`.
...
Cloudflare Pages Functions require a Pages-compatible local runtime if you need to test `/api/contact`, `/api/views`, or `/api/infinite-curios-latest` locally.
